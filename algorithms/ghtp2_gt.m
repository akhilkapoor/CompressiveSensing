%GHTP2_GT Graded Hard Thresholding Pursuit (with ground truth)
% GHTP2_GT(y,A,gt) runs the Hard Thresholding Pursuit started with x0=0 and with
% an size of the support increasing exponentially
% Usage: [ x,S,NormRes,NbIter, Ss, NormRess ] = ghtp2_gt( y, A, gt, x0, MaxNbIter, tolRes, verboseMode )
%   y: column vector, 
%   A: matrix, 
...
function [ x,S,NormRes,NbIter, Ss, NormRess ] = ghtp2_gt( y, A, gt, x0, MaxNbIter, tolRes, verboseMode )
...
while ( NbIter < MaxNbIter && (sum(ismember(Sgt,S)) ~= length(Sgt) ) && (norm(x-gt) > tolRes*normX) )
    ...
    S = sorted_idx(1:min(2^(NbIter+1),N)); % WE SHOULD DO THIS BETTER!!! -> Check in the earlier part of the code
    x=zeros(N,1); 
